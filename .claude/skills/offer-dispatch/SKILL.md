...
## Read this first — why every send used to have a new error (audit 2026-09-02)

The workflow had **50 saved versions in 33 days**, nearly all made *during* a send to get
past that day's error, and several of those edits caused the next send's error. Four
external dependencies also failed in the month with no check before sending (Resend key,
Anthropic credits, n8n execution quota, Airtable schema/records), and five one-shot send
workflows were built from scratch. Full evidence: [`AUDIT-2026-09-02.md`](AUDIT-2026-09-02.md).

**Rules that follow from it**

1. **Freeze on send days.** Never edit `Offer Dispatch — Akay` while a run is in flight or an
   offer is queued. If a send fails, fix the cause (credential, credits, data) and re-queue.
2. **Dependency check before queueing:** Resend key works (last run's responses carry ids, or
   send one pilot), today's Backup Registry row is *Verified*, n8n quota not exhausted, no
   Airtable schema change underway.
3. **No one-shot send workflows.** Every audience goes through this workflow. Missing
   targeting → add a field to the workflow on a non-send day, from the repo, with tests.
4. **One run at a time.** Do not trigger a second dispatch while one awaits approval.
5. **Edits: repo file → `node test-nodes.cjs` → paste → publish → verify `versionId ===
   activeVersionId`.** One named version per change. Never edit a node in the n8n UI.
...
Every Code node is kept in `n8n/` with a regression suite. Never edit them in the
n8n UI — edit the file, run the tests, then paste:

```bash
node test-nodes.cjs      # 38 tests: composer, recipients, HTML render+verify, sends, claim, halt
```

| File | Node | Notes |
|---|---|---|
| `n8n/gate-check.js` | Gate Check | mirrors the `Send Eligible` formula |
| `n8n/backup-check.js` | Backup Check | Verified backup dated today, Europe/Dublin |
| `n8n/build-recipients.js` | Build Recipients | audience, oldest-group-first, Excluded Countries |
| `n8n/claim-dispatch-group.js` | Claim Dispatch Group | unticks the queue flag before approval (2026-09-02) |
| `n8n/compose-email.js` | Compose Email | the composer; `compose-email.baseline.js` is the 2026-08-19 version the equivalence tests compare against |
| `n8n/render-html.js` | Render HTML | deterministic HTML (2026-09-02, replaces the Anthropic node) |
| `n8n/verify-html.js` | Verify HTML | proves the HTML adds nothing; falls back to text |
| `n8n/build-sends.js` | Build Sends | one item per recipient, `idempotencyKey` |
| `n8n/reconcile.js` | Reconcile | Sent Log rows, `_summary`, `_markBroadcasted` |
...
| Alert reads "no reason recorded" | Decline / all-failed paths carried no `haltReason` | **[fixed 2026-09-02]** reason recovered from Await Approval / Reconcile |
| Alert says "NO OFFERS WERE EMAILED" but clients have the mail | Fixed consequence text in the ERROR HANDLER | **[fixed 2026-09-02]** consequence now depends on the failing node |
| `Write Sent Log` 422 `Record ID … does not exist` after a full send | A Client record deleted between Fetch Clients and the log write | Since 2026-08-31 the node continues; the offer is still marked Broadcasted. Only that sub-batch of log rows is lost |
| `Execution limit reached` | n8n Cloud plan quota | Plan, not workflow. Check the plan on send days |
