...
if (bundleMembers.length > 1) {
  const AUDIENCE_KEYS = ['Target Capsule Tags', 'Target Countries', 'Bond/Customs Status', 'Match Interest Category', 'Category'];
...
return [{ json: {
  offerId: gate.offerId, bundleOfferIds, offerBond, targetTags, targetCountries, matchCategory,
  offerCategory: offerFields['Category'] || null, recipientCount: recipients.length, recipients,
  excludedCount: excluded.length, suppressedNoMailing, excluded,
...
function foldCountry(raw) {
  ...
  const ALIASES = { prchina:'china', cn:'china', roc:'taiwan', tw:'taiwan' };
  return ALIASES[s] || s;
}
function normalise(v) { const s = v && typeof v === 'object' ? v.name : v; return String(s ?? '').trim().toLowerCase(); }
function normaliseEmail(raw) { const s = String(raw ?? '').trim().toLowerCase(); return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(s) ? s : null; }
