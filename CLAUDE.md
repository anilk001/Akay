...
1. **Only public-safe fields reach the browser.** The `FIELDS` allowlist in
   ...
   video pipeline narrows the row a second time, through `VIDEO_FIELDS` in
   `video/video-fields.mjs` — the one list both the render CLI and the
   compositions import — so a new Airtable column can only reach a frame by a
