...
if (bundleMembers.length > 1) {
  // Report EVERY field the bundle disagrees on, not just the first. Each halt
  // unticks Queued for Dispatch, so returning on the first mismatch cost a
  // whole fix / re-tick / re-run cycle per field.
  ...
  for (const k of AUDIENCE_KEYS) {
    ...
    if (vals.size > 1) disagreements.push(`${k} (${[...vals].join(' vs ')})`);
  }
  if (disagreements.length) {
    return [{ json: { offerId: gate.offerId, bundleOfferIds: bundleMembers.map((m) => m.offerId), recipientCount: 0, recipients: [], halt: true, haltReason: `Bundle "${bundleId}" members disagree on ${disagreements.length} field(s), all of which must match before it can be sent as one email: ${disagreements.join('; ')}` } }];
  }
}
...
return [{ json: {
  ...
  skippedOffers: gate.skippedOffers || [],
